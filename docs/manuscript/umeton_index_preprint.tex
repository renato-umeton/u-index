...
\textbf{Single-author papers:} A single-author paper qualifies the author as first author; such papers are included in $L(R)$.

\textbf{Corresponding authorship:} Corresponding author status does not qualify, as practices vary substantially across fields and journals.

\textbf{Co-first and co-last authorship:} These designations are intentionally not taken into consideration. They are impossible to distinguish from the problematic incentive structure that this index aims to address. The U-index is designed to be simple and strict.
