...
- DOIs are normalized to lowercase for matching (OpenAlex returns lowercase, PubMed may have mixed case)
...
5. Results cached to SQLite (`~/.cache/uindex/cache.db`) with 7-day TTL
...
**Key modules:**
- `core.py` - Pure function `calculate_u_index()`, no I/O
- `pubmed.py` - PubMed E-utilities client, XML parsing, author position detection
- `openalex.py` - OpenAlex client, batches DOIs (50 at a time), returns citation counts
- `cache.py` - SQLite key-value store with TTL
- `cli.py` - Click-based CLI, orchestrates the pipeline
